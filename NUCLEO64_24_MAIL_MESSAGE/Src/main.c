...
osMailQId myMail01Handle; // untuk kirim dengan data type macam-macam
typedef struct {
	uint32_t voltage;
	uint32_t  current;
	uint32_t counter;
...
int main(void)
{
  ...
  osMailQDef(mailbox,10, mail_t);
  myMail01Handle= osMailCreate(osMailQ(mailbox),NULL);
...
void StartDefaultTask(void const * argument)
{
  ...
  {
	  i++;
	  mail=(mail_t*) osMailAlloc(myMail01Handle, osWaitForever);
//	  mail->voltage = i * 0.3;
//	  mail-> current= i * 0.11;
	  mail->voltage = i +2;
	  mail-> current= i +3;
	  mail-> counter =i;
	  osMailPut(myMail01Handle,mail);
	  osDelay(1000);
...
void StartTask02(void const * argument)
{
  ...
  for(;;)
  {
	  result= osMailGet(myMail01Handle,osWaitForever);
	  mail= (mail_t*) result.value.p;
	  memset(buff,0,sizeof(buff)); //bersihkan buffer
//	  sprintf(buff,"voltage: %.2f\n\rCurrent: %.2f\n\rCounter: %d\n\r",mail->voltage, mail->current, mail->counter);
	  sprintf(buff,"voltage: %d  Current: %d  Counter: %d \n\r",mail->voltage, mail->current, mail->counter);
	  HAL_UART_Transmit(&huart2,(uint8_t*)buff,sizeof(buff),1000);
	  osMailFree(myMail01Handle,mail);
//    osDelay(1); // ada os wait forever jadi gk perlu pakai delay
  }
